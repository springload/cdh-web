...
class TestStaffListView:
    ...
    def test_display_label_current(self, staffer):
        ...
        assert StaffListView().display_label(staffer) == staffer.positions.first().title
    ...
    def test_display_label_past(self, postdoc):
        ...
        position.end_date = timezone.now() - timedelta(days=30)
        position.save()
        assert StaffListView().display_label(postdoc) == "%s %s" % (
            position.years,
            position.title,
        )
...
class TestStudentListView:
    ...
    def test_list_current(self, client, student, staffer, grad_pi):
        ...
        assertContains(response, "%s Grant Recipient" % grad_pi.latest_grant.grant_type)
    ...
    def test_display_label_current(self, student, grad_pi, grad_pm):
        ...
        assert StudentListView().display_label(student) == str(
            student.positions.first().title
        )

        grant = grad_pi.latest_grant
        assert StudentListView().display_label(grad_pi) == "%s %s Grant Recipient" % (
            grant.years,
            grant.grant_type,
        )
    ...
    def test_display_label_past(self, student, grad_pi, grad_pm):
        ...
        position.end_date = timezone.now() - timedelta(days=30)
        position.save()
        assert StudentListView().display_label(student) == "%s %s" % (
            position.years,
            position.title,
        )
        ...
        membership.save()
        assert (
            StudentListView().display_label(grad_pm)
...
class TestAffiliateListView:
    ...
    def test_list(self, client, staffer, faculty_pi, staff_pi):
        ...
        assertContains(
            response,
            "{}–{} {} Grant Recipient".format(
                grant.start_date.year, grant.end_date.year, grant.grant_type.grant_type
            ),
            html=True,
        )
    ...
    def test_display_label(self, faculty_pi):
        ...
        assert AffiliateListView().display_label(
            faculty_pi
        ...
        grant_type.save()
        assert (
            AffiliateListView().display_label(faculty_pi)
...
class TestExecListView:
    ...
    def test_list_past(self, client, faculty_exec, staff_exec):
        ...
        for position in [faculty_exec.positions.first(), staff_exec.positions.first()]:
            position.end_date = timezone.now() - timedelta(days=30)
            position.save()
    ...
    def test_display_label(self):
        ...
        assert ExecListView().display_label(exec_member) == exec_member.job_title

...
class BlogListSitemap(Sitemap):
    ...
    def items(self):
        ...
        for date in live_posts.dates("first_published_at", "year", order="DESC"):
            items.append((date.year,))

        for date in live_posts.dates("first_published_at", "month", order="DESC"):
    ...
    def location(self, obj):
        ...
        if len(obj) == 2:
            return reverse(
                "blog:by-month", kwargs={"year": obj[0], "month": "%02d" % obj[1]}
            )

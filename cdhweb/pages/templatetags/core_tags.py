...
def _get_primary_nav_items():
    """
    Retrieve the main menu items as a queryset.
    """
    l1_menu_items = []
...
def secondary_nav_dict():
    ...
    """
    secondary_menu_items = _get_secondary_menu_items()
    cta_button = _get_secondary_nav_cta_button()
    secondary_menu_item_data = [
        _minor_menu_item_to_dict(item) for item in secondary_menu_items
    ]
    cta_button = [_minor_menu_item_to_dict(item) for item in cta_button]

    secondary_nav_data = {
        "secondary_nav": {
            "items": secondary_menu_item_data,
        },
    }
...
def _get_secondary_nav_cta_button():
    ...
    """
    secondary_menu = SecondaryNavigation.objects.prefetch_related("cta_button")

    if secondary_menu.exists():
        cta_button = secondary_menu.first().cta_button.all()
    return cta_button

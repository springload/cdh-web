...
class TestBlogListSitemap:
    ...
    def test_items(self, announcement):
        ...
        assert (announcement.first_published_at.year,
                announcement.first_published_at.month) in items

    def test_location(self):
        # named url
        assert BlogListSitemap().location(('list',)) == reverse('blog:list')
        # year only
        assert BlogListSitemap().location((2020,)) == \
            reverse('blog:by-year', args=[2020])
        # year + month; requires two-digit month
        assert BlogListSitemap().location((2019, 3)) == \
            reverse('blog:by-month', args=[2019, '03'])
    ...
    def test_lastmod(self, announcement, article):
        ...
        assert blogsitemap.lastmod(('list', )) \
            == announcement.first_published_at

        assert blogsitemap.lastmod((article.first_published_at.year, )) \
            == article.first_published_at

        assert blogsitemap.lastmod((article.first_published_at.year,
                                    article.first_published_at.month)) \
            == article.first_published_at

...
class TestGrant:
    ...
    def test_str(self, derrida, pliny):
        ...
        assert str(rpg) == "Derrida's Margins: Research Partnership (%s–%s)" % (
            rpg.start_date.year, rpg.end_date.year)
...
class TestMembership:
    ...
    def test_str(self, derrida, pliny):
        ...
        assert str(
            katie) == "Katie Chenoweth - Project Director on Derrida's Margins (%s)" % katie.years
        # check pliny membership with no end date
        ben = pliny.memberships.first()
        assert str(
            ben) == "Ben Hicks - Project Director on Pliny Project (%s)" % ben.years
